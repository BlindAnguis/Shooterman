...
TEST(EntityCreatorTests, createMage) {
  ...
  sf::Vector2f magePosition = sf::Vector2f(10, 50);
  int numberOfMageAnimations = 17;
  int mageMaxHealth = 100;
  ...
  EXPECT_EQ(velocityComponent->maxVelocity, sf::Vector2f(MAGE_MAX_VELOCITY, MAGE_MAX_VELOCITY));
  ...
  EXPECT_EQ(healthComponent->currentHealth, MAGE_MAX_HEALTH);
  EXPECT_TRUE(healthComponent->isAlive);
  EXPECT_EQ(healthComponent->maxHealth, MAGE_MAX_HEALTH);
  
  EXPECT_EQ(playerComponent->attackSpeed, MAGE_ATTACK_SPEED);
  ...
  EXPECT_EQ(manaComponent->currentMana, MAGE_MAX_MANA);
  EXPECT_EQ(manaComponent->maxMana, MAGE_MAX_MANA);
}

TEST(EntityCreatorTests, createKnight) {
  EntityCreator e = EntityCreator();
  sf::Vector2f knightPosition = sf::Vector2f(550, 230);
  int numberOfKnightAnimations = 17;
  int knightMaxHealth = 100;
  ...
  // Check that all components have the right values.
  EXPECT_EQ(velocityComponent->currentVelocity, sf::Vector2f(0, 0));
  EXPECT_EQ(velocityComponent->maxVelocity, sf::Vector2f(KNIGHT_MAX_VELOCITY, KNIGHT_MAX_VELOCITY));
  EXPECT_TRUE(velocityComponent->moveOnce);
  ...
  EXPECT_EQ(healthComponent->currentHealth, KNIGHT_MAX_HEALTH);
  EXPECT_TRUE(healthComponent->isAlive);
  EXPECT_EQ(healthComponent->maxHealth, KNIGHT_MAX_HEALTH);

  EXPECT_EQ(playerComponent->attackSpeed, KNIGHT_ATTACK_SPEED);
  EXPECT_FALSE(playerComponent->invinsible);
  EXPECT_EQ(playerComponent->nrOfDeaths, 0);
  EXPECT_EQ(playerComponent->nrOfKills, 0);
  ...
  EXPECT_EQ(staminaComponent->currentStamina, KNIGHT_MAX_STAMINA);
  EXPECT_EQ(staminaComponent->maxStamina, KNIGHT_MAX_STAMINA);
}
